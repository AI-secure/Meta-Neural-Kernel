...
class OmniglotNShot:
    ...
    def __init__(self, root, batchsz, n_way, k_shot, k_query, imgsz, device=None,n_train_tasks=1200,given_x=False,
        ...
        if not given_x:
            ...
            assert n_train_tasks <= 1200
            self.x_train, self.x_test = self.x[:n_train_tasks], self.x[1200:]
            self.normalization()
        else:
        ...
        self.batchsz = min(batchsz,self.x_train.shape[0])
        ...
        print("DB: train", self.x_train.shape)
        if self.x_test is not None:
            print("DB test", self.x_test.shape)

        self.datasets_cache = {"train": self.load_data_cache(self.datasets["train"],targets=self.y_train)}  # current epoch data cached
        if self.x_test is not None:
    ...
    def load_data_cache(self, data_pack, targets=None):
        """
        ...
        for sample in range(10):  # num of episodes
            ...
            for i in range(self.batchsz):  # one batch means one set
                ...
                for j, cur_class in enumerate(selected_cls):
                    ...
                    spt_idxes = selected_img[:self.k_shot]
                    qry_idxes = selected_img[self.k_shot:]
                    # meta-training and meta-test
                    x_spt.append(data_pack[cur_class][spt_idxes])
                    x_qry.append(data_pack[cur_class][qry_idxes])
                    if targets is None:
                        y_spt.append([j for _ in range(self.k_shot)])
                        y_qry.append([j for _ in range(self.k_query)])
                    else:
                        y_spt.append([targets[cur_class][spt_idxes]])
                        y_qry.append([targets[cur_class][qry_idxes]])
